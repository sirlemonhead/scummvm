 ...
 *
 * $URL: https://scummvm.svn.sourceforge.net/svnroot/scummvm/scummvm/trunk/engines/igor/igor.cpp $
 * $Id: igor.cpp 44284 2009-09-24 02:56:37Z thebluegr $
 *
 */
...
void IgorEngine::playSound(int num, int type) {
	...
	// TODO: Convert _sndFile to a stream
	Audio::AudioStream *stream = NULL; //Audio::makeVOCStream(_sndFile, Audio::FLAG_UNSIGNED);
	if (stream) {
...
void IgorEngine::updatePalette(int count) {
	...
	_system->getPaletteManager()->setPalette(pal, 0, count / 3);
}
...
void IgorEngine::setPaletteColor(uint8 index, uint8 r, uint8 g, uint8 b) {
	...
	pal[2] = (b << 2) | (b >> 4);
	pal[3] = 0;
	_system->getPaletteManager()->setPalette(pal, index, 1);
}
